...
"""
    ...
    def __call__(
        self, documents: Document | list[Document], temperature: float = 0.1
    ...
    async def __summarize_batch(
        self, documents: list[Document], temperature: float = 0.1
    ) -> list[Document]:
        ...
        summarized_documents = await self.__process_batch(
            documents, temperature, await_time_seconds=10
        )
        ...
        if documents_without_summaries:
            ...
            retry_results = await self.__process_batch(
                documents_without_summaries, temperature, await_time_seconds=30
            )
